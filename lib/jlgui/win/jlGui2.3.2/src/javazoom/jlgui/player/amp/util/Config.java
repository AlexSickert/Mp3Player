...
public class Config
{
	public static String[] protocols = {"http:","file:","ftp:",
										"https:","ftps:","jar:"};
	...
	private String _extensions = "m3u,wsz,pls,snd,aifc,aif,wav,au,mp1,mp2,mp3,ogg,spx";
	...
	public void load()
	{
		...
		if (_config.get(LAST_URL) == null)
			_config.add(LAST_URL, _lastUrl);
		if (_config.get(LAST_EQUALIZER) == null)
			_config.add(LAST_EQUALIZER, _lastEqualizer);
		if (_config.get(LAST_DIR) == null)
			_config.add(LAST_DIR, _lastDir);
		if (_config.get(TAGINFO_POLICY) == null)
			_config.add(TAGINFO_POLICY, _taginfoPolicy);
		if (_config.getInt(ORIGINE_X) == -1)
			_config.add(ORIGINE_X, _x);
		if (_config.getInt(ORIGINE_Y) == -1)
			_config.add(ORIGINE_Y, _y);
		if (_config.get(LAST_SKIN) == null)
			_config.add(LAST_SKIN, _defaultSkin);
		if (_config.get(LAST_PLAYLIST) == null)
			_config.add(LAST_PLAYLIST, _playlistFilename);
		if (_config.get(PLAYLIST_IMPL) == null)
			_config.add(PLAYLIST_IMPL, _playlist);
		if (_config.get(TAGINFO_MPEG_IMPL) == null)
			_config.add(TAGINFO_MPEG_IMPL, _taginfoMpeg);
		if (_config.get(TAGINFO_OGGVORBIS_IMPL) == null)
			_config.add(TAGINFO_OGGVORBIS_IMPL, _taginfoOggVorbis);
		if (_config.get(EXTENSIONS) == null)
			_config.add(EXTENSIONS, _extensions);
		if (_config.get(PROXY_SERVER) == null)
			_config.add(PROXY_SERVER, _proxyServer);
		if (_config.getInt(PROXY_PORT) == -1)
			_config.add(PROXY_PORT, _proxyPort);
		if (_config.getInt(VOLUME_VALUE) == -1)
			_config.add(VOLUME_VALUE, _volume);
		if (_config.get(PROXY_LOGIN) == null)
			_config.add(PROXY_LOGIN, _proxyLogin);
		if (_config.get(PROXY_PASSWORD) == null)
			_config.add(PROXY_PASSWORD, _proxyPassword);
		if (_config.getBoolean(PLAYLIST_ENABLED) == false)
			_config.add(PLAYLIST_ENABLED, _playlistEnabled);
		if (_config.getBoolean(SHUFFLE_ENABLED) == false)
			_config.add(SHUFFLE_ENABLED, _shuffleEnabled);
		if (_config.getBoolean(REPEAT_ENABLED) == false)
			_config.add(REPEAT_ENABLED, _repeatEnabled);
		if (_config.getBoolean(EQUALIZER_ENABLED) == false)
			_config.add(EQUALIZER_ENABLED, _equalizerEnabled);
		if (_config.getBoolean(EQUALIZER_ON) == false)
			_config.add(EQUALIZER_ON, _equalizerOn);
		if (_config.getBoolean(EQUALIZER_AUTO) == false)
			_config.add(EQUALIZER_AUTO, _equalizerAuto);
		if (_config.getBoolean(SCREEN_LIMIT) == false)
